# DENSE benchset — the project's own validation run

The original measurement of DENSE across six models, moved here from the repository README so
the README can stay short. **Nothing in this document has been re-measured or edited**; it is
the project's own reported data.

Six recent models — Opus 5, Opus 4.8, Sonnet 5, Sonnet 4.6, DeepSeek V4 Flash and Laguna
S 2.1 — benchmarked with no skill, with DENSE, and against two other terseness prompts,
**caveman** and **ponytail**. Everything below comes from executing model-written code against
hidden tests.

For independent runs against a verified clean control, see
[`CAVEMAN_BENCHSET.md`](CAVEMAN_BENCHSET.md) and [`AGENTIC_BENCH.md`](AGENTIC_BENCH.md).
